...
int Mamma(char* str)
{
	...
	appendTextToOutput( "MACRO PROCESSING CONCLUDED\n\n" );
	Mounty();
...
void ExpandOmori( char* macro, char* argInput )
{
	...
	while ( fgets( lineExpand, sizeof( lineExpand ), fileDeftab_g ) )
	{
		...
		if ( strcmp( expand.operation, "MEND" ) != 0 )
		{
			...
			strcat( guiout, expand.label );
			strcat( guiout, "\t" );
			strcat( guiout, expand.operation );
			strcat( guiout, "\t" );
			strcat( guiout, expand.operand );
			strcat( guiout, "\n" );
			appendTextToOutput( guiout );
